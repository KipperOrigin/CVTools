...
public abstract class Command
{
    ...
    public void addTextParameter(CommandParameterType type) {
	text.add(type);
    }
	...
	for(int i = commands.size(); i < args.length; i++) {
		...
		if(flags.contains(name)) {
		    ...
		    if(!text.get(textParametersSet).isValid(name)) return "Invalid parameter type!";
		    textParametersSet++;
		}
	    }
	    else {
		...
		if(optional.containsKey(name)) {
		    ...
		    if(!mandatory.get(name).isValid(par)) return "Wrong type of parameter " + name + "!";
		}
		else {
